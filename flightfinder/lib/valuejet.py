...
class ValueJet:
    ...
    def extract_data_from_item(self, item):
        ...
        try:
            ...
            time_24hr = datetime.strptime(formatted_time_am_pm, "%I:%M%p").strftime(
                "%H:%M"
            )

            destination_element = port_elements[1].find_all("span")
            destination_string = (
                destination_element[0].text if destination_element else None
            )
